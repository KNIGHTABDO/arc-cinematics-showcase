...
function MyListPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "favorites", filter: `profile_id=eq.${profileId}` },
        () => {
        ...
        <div className="mx-auto max-w-7xl px-[5vw]">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center pt-32 text-center">
              ...
              <p className="mt-3 text-sm text-arc-muted max-w-sm">{t("myList.emptyDesc", lang)}</p>
              <Link
                to="/browse"
                className="mt-6 bg-arc-accent text-arc-void px-6 py-3 rounded-full font-bold text-sm hover:bg-white transition"
              >
            ...
            >
              {movies.map((movie) => (
                <MovieCard
                  ...
                  movie={movie}
                  width={220}
                  linkPrefix={movie.isTV ? "/tv" : undefined}
                />
              ))}
            </div>
          )}
